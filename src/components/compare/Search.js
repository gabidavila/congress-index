  ...
  renderer = ({state, image, title, description, party }) => {
    return ([
      ...
      <div key='content' className='content'>
        {state && <Label circular style={{float: 'left', marginRight: '5px'}} color={parties[party].color}>{party}</Label>}
        ...
        <Grid.Column width={6}>
          {this.state.members.length ?
            <Segment>
              ...
              <br />
              <Button onClick={this.compareHandle} fluid color='teal'>Compare</Button>
            </Segment>
            : null}
        </Grid.Column>
